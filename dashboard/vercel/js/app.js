...
class DashboardApp {
  constructor() {
    ...
    this.latestPrices = {};
    const saved = localStorage.getItem('mulT_backend_url');
    const onVercel = /\.vercel\.app$/i.test(window.location.hostname);
    this.backendUrl = saved || (onVercel ? '' : window.location.origin);

    this.initElements();
    this.bindEvents();

    // On Vercel cloud host: require tunnel URL before connecting
    if (onVercel && !saved) {
      const modal = document.getElementById('configModal');
      if (modal) modal.classList.remove('hidden');
      if (this.el.connectionStatus) {
        this.el.connectionStatus.textContent = 'SET TUNNEL URL';
      }
    } else {
